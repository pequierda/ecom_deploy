...
// UTILITY FUNCTION: Get consistent today date
const getConsistentToday = () => {
...
export const softDeletePackage = async (packageId) => {
  ...
  try {
    ...
    // 2. Delete inclusions
    await connection.query(`DELETE FROM inclusions WHERE package_id = ?`, [
      packageId,
    ]);
...
/* =======================
   PREPARATION DAYS HELPER FUNCTIONS - UPDATED
======================= */
...
// CORRECTED: Check if a date falls within preparation period
export const isDateInPreparationPeriod = async (package_id, date) => {
  try {
    ...
    // Calculate the preparation period: tomorrow through (today + preparationDays)
    const prepStartDate = new Date(today);
    prepStartDate.setDate(today.getDate() + 1); // Start from tomorrow
    
    // FIXED: End date should be today + preparation days (inclusive)
    const prepEndDate = new Date(today);
    prepEndDate.setDate(today.getDate() + preparationDays);

    console.log(`Preparation period check for package ${package_id}:`);
    console.log(`Today: ${today.toISOString().split('T')[0]}`);
    console.log(`Target date: ${targetDate.toISOString().split('T')[0]}`);
    console.log(`Prep start: ${prepStartDate.toISOString().split('T')[0]}`);
    console.log(`Prep end: ${prepEndDate.toISOString().split('T')[0]}`);
    console.log(`Preparation days: ${preparationDays}`);

    // Block dates from tomorrow up to and including the preparation end date
    const isInPeriod = targetDate >= prepStartDate && targetDate <= prepEndDate;
    console.log(`Is in preparation period: ${isInPeriod}`);
...
export const checkDateAvailability = async (packageId, date) => {
  try {
    ...
    // 2. Check for preparation period (application level) - UPDATED
    const inPreparationPeriod = await isDateInPreparationPeriod(
      packageId,
      date
    );
    if (inPreparationPeriod) {
      ...
      return {
        available: false,
        reason: "This date falls within the preparation period",
        totalSlots: 0,
        bookedSlots: 0,
        availableSlots: 0,
        isBlocked: true,
...
// COMPLETELY REWRITTEN: Get availability for a range of dates
export const getPackageAvailabilityRange = async (
  ...
  try {
    console.log('Starting getPackageAvailabilityRange with:', {
      packageId,
      startDate,
      endDate,
    ...
    if (!defaultSlots) {
      console.log('No default slots found for package');
      return {};
    }

    const preparationDays = await getPackagePreparationDays(packageId);
    console.log(`Package has ${preparationDays} preparation days`);
    ...
    // Fetch manually unavailable dates
    ...
    // Fetch date-specific slot configurations
    ...
    // Step 1: Mark manually unavailable dates
    unavailableDates.forEach((item) => {
      availabilityMap[item.unavailable_date] = {
      ...
      console.log(`Marked ${item.unavailable_date} as manually unavailable`);
    });

    // Step 2: CORRECTED - Calculate and mark preparation period dates
    if (preparationDays > 0) {
      // Calculate preparation period: tomorrow through (today + preparationDays)
      const prepStartDate = new Date(today.getTime() + (1 * 24 * 60 * 60 * 1000)); // Tomorrow
      const prepEndDate = new Date(today.getTime() + (preparationDays * 24 * 60 * 60 * 1000)); // Today + prep days
      
      const prepStartString = prepStartDate.toISOString().split('T')[0];
      const prepEndString = prepEndDate.toISOString().split('T')[0];
      
      console.log(`CORRECTED CALCULATION:`);
      console.log(`Today: ${todayString}`);
      console.log(`Preparation days: ${preparationDays}`);
      console.log(`Prep period: ${prepStartString} to ${prepEndString} (inclusive)`);
      
      // Generate all dates in the preparation period
      let current = new Date(prepStartDate);
      const preparationDates = [];
      
      while (current <= prepEndDate) {
        const dateStr = current.toISOString().split('T')[0];
        
        // CRITICAL: Double-check we're never including today
        if (dateStr === todayString) {
          console.log(`ERROR: Trying to mark today (${dateStr}) as prep period - SKIPPING`);
        } else {
          preparationDates.push(dateStr);
        }
        
        current = new Date(current.getTime() + (24 * 60 * 60 * 1000));
      }
      
      console.log(`Generated preparation dates: ${preparationDates.join(', ')}`);
      
      // Mark preparation period dates within our requested range
      preparationDates.forEach(dateStr => {
        if (dateStr >= startDate && dateStr <= endDate) {
          // Don't override manually blocked dates
          if (!availabilityMap[dateStr] || !availabilityMap[dateStr].isBlocked) {
            availabilityMap[dateStr] = {
              ...
              isBlocked: true,
              isPreparationPeriod: true,
              reason: `Requires ${preparationDays} preparation days`,
            };
            console.log(`Marked ${dateStr} as preparation period`);
          } else {
            console.log(`Skipping ${dateStr} - already manually blocked`);
          }
        } else {
          console.log(`Prep date ${dateStr} is outside requested range (${startDate} to ${endDate})`);
        }
      });
    }

    // Step 3: Add date-specific availability data
    dateSpecificData.forEach((item) => {
      const dateStr = item.available_date;
      const availableSlots = item.total_slots - item.booked_slots;
      
      // Check if this date is already marked as preparation period or manually blocked
      if (availabilityMap[dateStr] && 
          (availabilityMap[dateStr].isPreparationPeriod || 
           (availabilityMap[dateStr].isBlocked && !availabilityMap[dateStr].isPreparationPeriod))) {
        // Don't override blocked/preparation dates
        console.log(`Skipping override for blocked/prep date: ${dateStr}`);
        return;
      }
      ...
      console.log(`Added specific availability for ${dateStr}: ${availableSlots}/${item.total_slots} slots`);
    });

    // Step 4: Fill in remaining dates in the range
    ...
    while (current <= endDateObj) {
      ...
      if (!availabilityMap[dateStr]) {
        // Determine what this date should be
        if (current < today) {
          // This is a past date - skip it entirely
          console.log(`Skipping past date: ${dateStr}`);
        } else if (current.getTime() === today.getTime()) {
          // This is today - not available but NOT preparation period
          availabilityMap[dateStr] = {
            totalSlots: defaultSlots,
            bookedSlots: 0,
            availableSlots: 0,
            available: false,
            isBlocked: true,
            isPreparationPeriod: false, // CRITICAL: Today is never preparation period
            reason: "Today is not available for booking",
          };
          console.log(`Marked today (${dateStr}) as unavailable but NOT preparation period`);
        } else {
          // This is a future date - use the corrected preparation period check
          const isInPrepPeriod = await isDateInPreparationPeriod(packageId, dateStr);
          ...
          if (isInPrepPeriod) {
            ...
            console.log(`Future date ${dateStr} is in preparation period`);
          } else {
            availabilityMap[dateStr] = {
              totalSlots: defaultSlots,
              bookedSlots: 0,
            ...
            console.log(`Future date ${dateStr} is available`);
          }
        }
      }
    ...
    // FINAL STEP: Filter out past dates
    const filteredMap = {};
    Object.entries(availabilityMap).forEach(([dateStr, availability]) => {
      const dateObj = new Date(dateStr + 'T00:00:00');
      
      if (dateObj >= today) {
        filteredMap[dateStr] = availability;
      } else {
        console.log(`FINAL FILTER: Removing past date ${dateStr}`);
      }
    });

    console.log(`Final availability map for package ${packageId}:`);
    Object.entries(filteredMap).forEach(([date, avail]) => {
      if (avail.isPreparationPeriod) {
        console.log(`  ${date}: PREPARATION PERIOD`);
      } else if (avail.isBlocked) {
        console.log(`  ${date}: BLOCKED - ${avail.reason}`);
      } else if (avail.available) {
        console.log(`  ${date}: AVAILABLE (${avail.availableSlots}/${avail.totalSlots})`);
      } else {
        console.log(`  ${date}: UNAVAILABLE - ${avail.reason}`);
      }
    });
    ...
    console.error("Error in getPackageAvailabilityRange:", error);
    throw error;
  }
};
// UPDATED: Get upcoming available dates excluding preparation periods
export const getUpcomingAvailableDates = async (
  packageId,
  daysAhead = 30,
  limit = 7
) => {
