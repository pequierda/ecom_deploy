...
// Get "today" in +8 timezone
const getLocalToday = () => {
  const now = new Date();
  const offsetMs = 8 * 60 * 60 * 1000; // +8 hours
  const local = new Date(now.getTime() + offsetMs);
  local.setUTCHours(0, 0, 0, 0); // midnight in +8
  return local;
};
...
const BookingCalendar = ({
 ...
 const isDateAvailable = (date: Date) => {
  // Can't book past dates (but allow today)
  if (isPastDate(date) && !isToday(date)) {
    return false;
  }

  // Check availability data from API
  const dateStr = formatDate(date);
  const dayAvailability = availabilityData[dateStr];

  if (!dayAvailability) {
    // For dates without API data, only assume available if they're far enough in the future
    // This prevents issues with trailing days from previous months
    const today = new Date();
    const daysDifference = Math.ceil((date.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
    
    // If it's within the preparation period or very soon, treat as unavailable
    return daysDifference > (preparationDays || 0);
  }

  // Date is available if not blocked and has slots
  return (
 ...
 const getDateStatus = (date: Date) => {
    ...
    // Handle today separately
    if (isToday(date)) return "today";

    // Check preparation period
    if (dayAvailability?.isPreparationPeriod) {
      return "preparation";
    }

    if (!dayAvailability) {
      // Apply preparation days rule if no specific data
      if (preparationDays > 0) {
        const today = new Date();
        today.setHours(0, 0, 0, 0);

        const prepEnd = new Date(today);
        prepEnd.setDate(prepEnd.getDate() + preparationDays);

        // Only mark as preparation if date is AFTER today and within prep period
        if (date > today && date <= prepEnd) {
          return "preparation";
        }
      }
      return "available";
    ...
    if (dayAvailability.isBlocked && !dayAvailability.isPreparationPeriod)
      return "blocked";
    if (dayAvailability.availableSlots === 0) return "full";
    if (
      dayAvailability.availableSlots <=
      Math.ceil(dayAvailability.totalSlots * 0.2)
    )
      return "limited";
    return "available";
  };
  ...
  useEffect(() => {
    const fetchAvailability = async () => {
      ...
      try {
        ...
        console.log("🔍 Fetching availability for range:", {
          startDate,
          endDate,
          preparationDays,
        });
    ...
    >
        ...
        <div className="grid grid-cols-7 gap-1 mb-6">
          {days.map((day, index) => {
            ...
            const handleDateClick = () => {
              if (canSelect) {
                ...
                if (isPastDate(day)) {
                  ...
                  alert(
                    `This date requires ${preparationDays} preparation day${
                      preparationDays !== 1 ? "s" : ""
                    }. The next available date is ${getNextAvailableDate()}.`
                  );
                ...
                } else if (availability?.availableSlots === 0) {
                  alert(
            ...
            const getNextAvailableDate = () => {
              ...
              nextAvailable.setDate(today.getDate() + preparationDays + 1);
              return nextAvailable.toLocaleDateString("en-US", {
                weekday: "long",
                year: "numeric",
                month: "long",
                day: "numeric",
            ...
            const getTooltip = () => {
              ...
              if (availability?.isPreparationPeriod) {
                return `Preparation period: This date requires ${preparationDays} day${
                  preparationDays !== 1 ? "s" : ""
                } advance booking`;
              }
              if (
              ...
              return `${availability?.availableSlots || defaultSlots} of ${
                availability?.totalSlots || defaultSlots
              } slots available`;
            };
                ...
                <div className="flex flex-col items-center justify-center h-full p-1">
                  ...
                  {availability.isPreparationPeriod && (
                    <div className="flex items-center space-x-1 text-xs text-orange-600 font-medium mt-1">
                      ...
                      <span>N/A</span>{" "}
                      {/* Changed from "Prep" to be consistent */}
                    </div>
                  )}
                  ...
                  {availability.isBlocked &&
                    !availability.isPreparationPeriod && (
                      <span className="text-xs text-red-500 font-medium flex items-center space-x-1 mt-1">
                        ...
                        <span>N/A</span>{" "}
                        {/* Only show N/A for manually blocked dates, not preparation */}
                      </span>
                    )}
      ...
      <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
        ...
        <div className="space-y-1 text-xs text-blue-600">
          <p>
            Bookings must be made at least 1 day in advance. Today's date is not
            available for booking.
          </p>
          {preparationDays > 0 ? (
            <p className="font-medium">
              This package requires {preparationDays} preparation day
              {preparationDays !== 1 ? "s" : ""}. Dates within the next{" "}
              {preparationDays} day{preparationDays !== 1 ? "s" : ""} are not
              available for booking.
            </p>
          ) : (
            <p>
              No preparation period required. Dates are available starting from
              tomorrow.
            </p>
          )}
        </div>
    ...
    const fetchPackagePreparationDays = async () => {
      if (currentPackage) {
        ...
        try {
          ...
          console.log(
            "📅 Fetched preparation days:",
            prepDaysInfo.preparationDays
          );
        } catch (error) {
          ...
          setPreparationDaysInfo({
            ...
            error:
              error instanceof Error
                ? error.message
                : "Failed to fetch preparation days",
          });
        }
      }
    };
                    ...
                    <div className="flex items-center justify-between mb-4">
                      ...
                      {preparationDaysInfo.preparationDays > 0 && (
                        <div className="flex items-center text-sm text-blue-600 bg-blue-50 px-3 py-1 rounded-full">
                          ...
                          <span>
                            {preparationDaysInfo.preparationDays} prep day
                            {preparationDaysInfo.preparationDays !== 1
                              ? "s"
                              : ""}{" "}
                            after booking
                          </span>
                  ...
                  <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                    ...
                    <div className="text-xs text-blue-600 space-y-1">
                      <p>
                        ...
                        {currentPackage.defaultSlots !== 1 ? "s" : ""} by
                        default. Some dates may have custom allocations.
                      </p>
                      {/* NEW: Show preparation days info */}
                      {preparationDaysInfo.preparationDays > 0 && (
